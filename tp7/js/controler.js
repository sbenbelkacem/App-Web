...
meteoControllers.controller('MainController', ['$scope', '$http',
	...
	var appid ="appid=7c44d4c0f8dcb85670b322771e9b3845" ; 
		...
		$scope.recherche = function() {
			...
			$http.get('https://demo.bilelz.fr/owmap/?q='+$scope.city+'&'+unit + '&'+appid)
			...
			$http.get('https://demo.bilelz.fr/owmap/forecast/?q='+$scope.city+'&'+unit+ '&'+appid+'&lang=fr')
				.success(function(data) {
					$scope.forecast = data;
		...
		function showPosition(position) {
			/* appel AJAX à l’API openweathermap */	   
			$http.get('https://demo.bilelz.fr/owmap/?lat='+position.coords.latitude+'&lon='+position.coords.longitude+'&'+unit + '&'+appid)
				.success(function(data) {
				/* on met dans l’objet meteo les données retournées par openweatherapp */
					$scope.meteo = data;
				}).error(function(data) {
